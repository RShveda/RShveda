...
document.addEventListener("DOMContentLoaded", function(event) {
    ...
    // actions which trigger search (and listing refresh)

    // Create HTTP request object
    let searchXhr = new XMLHttpRequest();

    searchXhr.onloadstart = function () {
        // Show loading screen
        showLoadingScreen();
    };

    searchXhr.onloadend = function () {
        // Hide loading screen
        hideLoadingScreen();
    };
    ...
    searchXhr.onload = function () {
        ...
        if (searchXhr.status >= 200 && searchXhr.status < 300) {
            // Success
            if (searchXhr.responseType == 'json') {
                response = JSON.parse(searchXhr.response);
                refreshListing();
            }
        } else {
            // Failure
    ...
    function runSearch() {
        ...
        // Retrieve the location selector which is not in the search form
        const locationSelector = document.getElementById('location-selector');

        // Build a formatted search parameters string for POST url
        const formattedSearchParameters = serialize(searchForm) + locationSelector.choices.getFormattedValues();

        // Send the search request
        searchXhr.open('POST', urls.advertsListingController.searchAdvertsViewComponent);
        searchXhr.send(formattedSearchParameters);
    }

    // Refresh the listing
    function refreshListing(listingHtml) {
        document.getElementById('listing').innerHTML = listingHtml;
    }

    // Bind the controls that trigger the search here :
